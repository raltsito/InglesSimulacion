...
BEGIN
    ...
    IF @estado <> 'EnCurso'
    BEGIN
        RAISERROR('Solo se pueden cancelar ex�menes en curso.', 16, 1);
        RETURN;
    END
...
BEGIN
    ...
    BEGIN
        RAISERROR('El usuario ya alcanz� el m�ximo de 2 intentos finales', 16, 1);
        RETURN;
    END
...
AS 
	BEGIN
		...
		BEGIN
			RAISERROR('El usuario ya tiene un examen de pr�ctica en curso', 16, 1);
			RETURN;
		END
		...
		BEGIN
			RAISERROR('El usuario ya alcanz� el m�ximo de 5 intentos de pr�ctica', 16, 1);
			RETURN;
		END

		IF (SELECT COUNT(*) FROM preguntas WHERE activa = 1) < 20
		BEGIN
			RAISERROR('No hay suficientes preguntas activas para generar el examen de pr�ctica', 16, 1);
			RETURN;
		END
